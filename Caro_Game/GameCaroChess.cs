using System;
using System.Collections.Generic;
using System.Linq;

using System.Drawing;
using System.Windows.Forms;

namespace Caro_Game
{
    enum Winner { PLAYER1, PLAYER2, NOONE, BOT, NONE }
    public class GameCaroChess
    {
        Chessboard _Chesssboard;
        Graphics _grs;
        Stack<Chess_BoardInfo> _stackChess;
        Winner _gameOver;
        int turn;
        int mod;
        string nameP1;
        string nameP2;
        List<List<Chess_BoardInfo>> _listChessBoard;
        public Chessboard chessboard { get => _Chesssboard; set => _Chesssboard = value; }
        public Graphics Grs { get => _grs; }
        public int Turn { get => turn; set => turn = value; }
        internal Winner GameOver { get => _gameOver; }
        public int Mod { get => mod; set => mod = value; }
        public string NameP1 { get => nameP1; set => nameP1 = value; }
        public string NameP2 { get => nameP2; set => nameP2 = value; }
        internal Stack<Chess_BoardInfo> StackChess { get => _stackChess; set => _stackChess = value; }

        public GameCaroChess(Graphics grs, string namePlayer1, string namePlayer2)
        {
            this._grs = grs;
            chessboard = new Chessboard(Grs);
            StackChess = new Stack<Chess_BoardInfo>();

            turn = 1;
            _gameOver = Winner.NONE;
            Mod = 2;
            NameP1 = namePlayer1;
            NameP2 = namePlayer2;
            _listChessBoard = chessboard.ListChessBoard;
        }


        public void drawChessBoard()
        {
            chessboard.drawChessboard();
          
        }
        void load_CheckBoard()
        {
            chessboard.load_CheckBoard();
        }
        bool chess(int Mouse_X, int Mouse_Y)
        {
            if (Mouse_X % MyConst._ChessBoard_SizeWitdth == 0 ||
                Mouse_Y % MyConst._ChessBoard_SizeHeight == 0)
            {
                return false;
            }
            int column = Mouse_X / MyConst._ChessBoard_SizeWitdth;
            int row = Mouse_Y / MyConst._ChessBoard_SizeHeight;
            // Chess_BoardInfo tmp = _Chesssboard.ListChessBoard[row][column];
            if (_listChessBoard[row][column].Ownership != 0)
                return false;

            _listChessBoard[row][column].Ownership = turn;

            _listChessBoard[row][column].FileImage = _listChessBoard[row][column].Ownership == 1 ? MyConst.fileName_X : MyConst.fileName_O;
            drawChess(_listChessBoard[row][column]);
            StackChess.Push(_listChessBoard[row][column]);
            turn = turn != 1 ? 1 : 2;
            return true;
        }

        void drawChess(Chess_BoardInfo cbi)
        {
            if (cbi.Ownership == 0)
                return;

            string fileName;
            fileName = cbi.FileImage;

            Bitmap bm = new Bitmap(fileName);
            _Chesssboard.drawChess_Board(cbi.Location, bm);
        }
        public void drawChessBoardRefesh()
        {
            foreach (Chess_BoardInfo item in StackChess)
            {
                drawChess(item);
            }
        }


        #region Xử lí thắng thua
        void win()
        {
            string nof = "";
            switch (_gameOver)
            {
                case Winner.PLAYER1:
                    nof = string.Format("'{0}' Thắng", nameP1);
                    break;
                case Winner.PLAYER2:
                    nof = string.Format("'{0}' Thắng", nameP2);
                    break;
                case Winner.NOONE:
                    nof = "Hòa";
                    break;
                case Winner.BOT:
                    nof = "Computer thắng";
                    break;
                default:
                    return;
            }
            MessageBox.Show(nof, "GameOver");
            
        }
        public bool checkWin()
        {
            if (StackChess.Count == MyConst._NumRow * MyConst._NumColumn)
            {
                _gameOver = Winner.NOONE;
                turn = 3;
                return true;
            }


            foreach (Chess_BoardInfo item in StackChess)
            {
                if (browseAlong(item.Index_Column, item.Index_Row, item.Ownership) ||
              browseHorizontal(item.Index_Column, item.Index_Row, item.Ownership) ||
              browseMainDiagonal(item.Index_Column, item.Index_Row, item.Ownership) ||
              browseExtraDiagonal(item.Index_Row, item.Index_Column, item.Ownership))
                {
                    _gameOver = turn != 1 ? Winner.PLAYER1 : Winner.PLAYER2;
                    return true;
                }
            }
            return false;

        }

        bool browseAlong(int currIndex_C, int currIndex_R, int currPlayer)
        {
            if (currIndex_R > MyConst._NumRow - 5)
                return false;
            int cout = 1;
            for (cout = 1; cout < 5; cout++)
            {
                if (_listChessBoard[currIndex_R + cout][currIndex_C].Ownership != currPlayer)
                    return false;
            }
            if (currIndex_R == 0 || currIndex_R + cout == MyConst._NumRow)
                return true;
            if (_listChessBoard[currIndex_R + cout][currIndex_C].Ownership == 0 || _listChessBoard[currIndex_R - 1][currIndex_C].Ownership == 0)
                return true;

            return false;
        }
        bool browseHorizontal(int currIndex_C, int currIndex_R, int currPlayer)
        {
            if (currIndex_C > MyConst._NumColumn - 5)
                return false;
            int cout = 1;
            for (cout = 1; cout < 5; cout++)
            {
                if (_listChessBoard[currIndex_R][currIndex_C + cout].Ownership != currPlayer)
                    return false;
            }
            if (currIndex_C == 0 || currIndex_C + cout == MyConst._NumColumn)
                return true;
            if (_listChessBoard[currIndex_R][currIndex_C + cout].Ownership == 0 || _listChessBoard[currIndex_R][currIndex_C - 1].Ownership == 0)
                return true;

            return false;

        }
        bool browseMainDiagonal(int currIndex_C, int currIndex_R, int currPlayer)
        {
            if (currIndex_C > MyConst._NumColumn - 5 || currIndex_R > MyConst._NumRow - 5)
                return false;
            int cout = 1;
            for (cout = 1; cout < 5; cout++)
            {
                if (_listChessBoard[currIndex_R + cout][currIndex_C + cout].Ownership != currPlayer)
                    return false;
            }
            if (currIndex_C + cout == MyConst._NumColumn || currIndex_R + cout == MyConst._NumRow || currIndex_R == 0 || currIndex_C == 0)
                return true;
            if (_listChessBoard[currIndex_R + cout][currIndex_C + cout].Ownership == 0 || _listChessBoard[currIndex_R - 1][currIndex_C - 1].Ownership == 0)
                return true;

            return false;

        }
        bool browseExtraDiagonal(int currIndex_R, int currIndex_C, int currPlayer)
        {

            if (currIndex_R < 4 || currIndex_C > MyConst._NumColumn - 5)
                return false;
            int cout = 1;
            for (cout = 1; cout < 5; cout++)
            {
                if (_listChessBoard[currIndex_R - cout][currIndex_C + cout].Ownership != currPlayer)
                    return false;
            }
            if (currIndex_R == 4 || currIndex_C + cout == MyConst._NumColumn || currIndex_C == 0 || currIndex_R == MyConst._NumRow - 1)
                return true;
            if (_listChessBoard[currIndex_R - cout][currIndex_C + cout].Ownership == 0 || _listChessBoard[currIndex_R + 1][currIndex_C - 1].Ownership == 0)
                return true;
            return false;

        }

        #endregion
        void loadInfo(Label name, PictureBox ptb)
        {
            if (turn == 1)
            {
                name.Text = NameP1;
                ptb.BackgroundImage = new Bitmap(MyConst.fileName_X);
            }
            else
            {
                name.Text = NameP2;
                ptb.BackgroundImage = new Bitmap(MyConst.fileName_O);
            }
        }
        public void startGame(Label name, PictureBox ptb)
        {
            load_CheckBoard();
            if (mod == 1)
                startComputer();
            loadInfo(name, ptb);

        }
        public void newGame(Panel pnlChessboard, Label name, PictureBox ptb, Button btn)
        {
            StackChess.Clear();

            turn = 1;
            _grs.Clear(pnlChessboard.BackColor);
            drawChessBoard();
            load_CheckBoard();
            _gameOver = Winner.NONE;
            name.Text = "";
            ptb.BackgroundImage = null;
            btn.Visible = true;

        }
        #region AI
        public long[] arrayDef = new long[7] { 0, 3, 27, 99, 729, 6561, 59049 };
        public long[] arrayAtt = new long[7] { 0, 9, 54, 162, 1458, 15112, 118008 };
        public void startComputer()
        {
            if (StackChess.Count() == 0)
            {
                turn = 1;
                chess(((MyConst._NumColumn - 1) / 2) * MyConst._ChessBoard_SizeHeight + 1, ((MyConst._NumRow - 1) / 2) * MyConst._ChessBoard_SizeWitdth + 1);
                return;
            }
            else
            {
                Chess_BoardInfo chess_Board = timKiemNuocDi();
                if (!chess(chess_Board.Location.X + 2, chess_Board.Location.Y + 2))
                {
                    MessageBox.Show("Lỗi");
                }
            }
        }

        Chess_BoardInfo timKiemNuocDi()
        {
            Chess_BoardInfo rsl = new Chess_BoardInfo();
            long numMax = 0;
            for (int i = 0; i < MyConst._NumRow; i++)
            {
                for (int j = 0; j < MyConst._NumColumn; j++)
                {
                    if (_listChessBoard[i][j].Ownership == 0)
                    {
                        long diemTanCong = diemTCDoc(i, j) + diemTCNgang(i, j) + diemTCCheoChinh(i, j) + diemTCCheoPhu(i, j);
                        long diemPhongNgu = diemPTDoc(i, j) + diemPTNgang(i, j) + diemPTCheoChinh(i, j) + diemPTCheoPhu(i, j);
                        long diemTmp = diemTanCong > diemPhongNgu ? diemTanCong : diemPhongNgu;
                        if (diemTmp > numMax)
                        {
                            numMax = diemTmp;
                            rsl = new Chess_BoardInfo(_listChessBoard[i][j].Index_Row, _listChessBoard[i][j].Index_Column, _listChessBoard[i][j].Location, 0);
                        }
                    }
                }
            }
            return rsl;
        }

        #region Tấn công
        long diemTCDoc(int currRow, int currColumn)
        {
            long diemTong = 0;
            int soQuanDich = 0;
            int soQuanTa = 0;
            for (int i = 1; i < 6 && currRow + i < MyConst._NumRow; i++)
            {
                if (_listChessBoard[currRow + i][currColumn].Ownership == 1)
                {
                    soQuanTa++;
                }
                else if (_listChessBoard[currRow + i][currColumn].Ownership == 2)
                {
                    soQuanDich++;
                    break;
                }
                else
                    break;
            }

            for (int i = 1; i < 6 && currRow - i >= 0; i++)
            {
                if (_listChessBoard[currRow - i][currColumn].Ownership == 1)
                {
                    soQuanTa++;
                }
                else if (_listChessBoard[currRow - i][currColumn].Ownership == 2)
                {
                    soQuanDich++;
                    break;
                }
                else
                    break;
            }
            if (soQuanDich == 2)
                return 0;
            diemTong -= arrayDef[soQuanDich] * 19;
            diemTong += arrayAtt[soQuanTa];
            return diemTong;
        }
        long diemTCNgang(int currRow, int currColumn)
        {
            long diemTong = 0;
            int soQuanDich = 0;
            int soQuanTa = 0;
            for (int i = 1; i < 6 && currColumn + i < MyConst._NumColumn; i++)
            {
                if (_listChessBoard[currRow][currColumn + i].Ownership == 1)
                {
                    soQuanTa++;
                }
                else if (_listChessBoard[currRow][currColumn + i].Ownership == 2)
                {
                    soQuanDich++;
                    break;
                }
                else
                    break;
            }

            for (int i = 1; i < 6 && currColumn - i >= 0; i++)
            {
                if (_listChessBoard[currRow][currColumn - i].Ownership == 1)
                {
                    soQuanTa++;
                }
                else if (_listChessBoard[currRow][currColumn - i].Ownership == 2)
                {
                    soQuanDich++;
                    break;
                }
                else
                    break;
            }
            if (soQuanDich == 2)
                return 0;
            diemTong -= arrayDef[soQuanDich] *19;
            diemTong += arrayAtt[soQuanTa];
            return diemTong;
        }
        long diemTCCheoChinh(int currRow, int currColumn)
        {
            long diemTong = 0;
            int soQuanDich = 0;
            int soQuanTa = 0;
            for (int i = 1; i < 6 && currRow + i < MyConst._NumRow && currColumn + i < MyConst._NumColumn; i++)
            {
                if (_listChessBoard[currRow + i][currColumn + i].Ownership == 1)
                {
                    soQuanTa++;
                }
                else if (_listChessBoard[currRow + i][currColumn + i].Ownership == 2)
                {
                    soQuanDich++;
                    break;
                }
                else
                    break;
            }

            for (int i = 1; i < 6 && currRow - i >= 0 && currColumn - i >= 0; i++)
            {
                if (_listChessBoard[currRow - i][currColumn - i].Ownership == 1)
                {
                    soQuanTa++;
                }
                else if (_listChessBoard[currRow - i][currColumn - i].Ownership == 2)
                {
                    soQuanDich++;
                    break;
                }
                else
                    break;
            }
            if (soQuanDich == 2)
                return 0;
            diemTong -= arrayDef[soQuanDich ]*19;
            diemTong += arrayAtt[soQuanTa];
            return diemTong;
        }
        long diemTCCheoPhu(int currRow, int currColumn)
        {
            long diemTong = 0;
            int soQuanDich = 0;
            int soQuanTa = 0;
            for (int i = 1; i < 6 && currColumn + i < MyConst._NumColumn && currRow - i >= 0; i++)
            {
                if (_listChessBoard[currRow - i][currColumn + i].Ownership == 1)
                {
                    soQuanTa++;
                }
                else if (_listChessBoard[currRow - i][currColumn + i].Ownership == 2)
                {
                    soQuanDich++;
                    break;
                }
                else
                    break;
            }

            for (int i = 1; i < 6 && currRow + i < MyConst._NumRow && currColumn - i >= 0; i++)
            {
                if (_listChessBoard[currRow + i][currColumn - i].Ownership == 1)
                {
                    soQuanTa++;
                }
                else if (_listChessBoard[currRow + i][currColumn - i].Ownership == 2)
                {
                    soQuanDich++;
                    break;
                }
                else
                    break;
            }
            if (soQuanDich == 2)
                return 0;
            diemTong -= arrayDef[soQuanDich ]*19;
            diemTong += arrayAtt[soQuanTa];
            return diemTong;
        }
        #endregion
        #region Phòng thủ
        long diemPTDoc(int currRow, int currColumn)
        {
            long diemTong = 0;
            int soQuanDich = 0;
            int soQuanTa = 0;
            for (int i = 1; i < 6 && currRow + i < MyConst._NumRow; i++)
            {
                if (_listChessBoard[currRow + i][currColumn].Ownership == 1)
                {
                    soQuanTa++;
                    break;
                }
                else if (_listChessBoard[currRow + i][currColumn].Ownership == 2)
                {
                    soQuanDich++;
                }
                else
                    break;
            }

            for (int i = 1; i < 6 && currRow - i >= 0; i++)
            {
                if (_listChessBoard[currRow - i][currColumn].Ownership == 1)
                {
                    soQuanTa++;
                    break;
                }
                else if (_listChessBoard[currRow - i][currColumn].Ownership == 2)
                {
                    soQuanDich++;
                }
                else
                    break;
            }
            if (soQuanTa == 2)
                return 0;
            diemTong += arrayDef[soQuanDich];
          //  diemTong -= arrayAtt[soQuanTa];
            return diemTong;
        }
        long diemPTNgang(int currRow, int currColumn)
        {
            long diemTong = 0;
            int soQuanDich = 0;
            int soQuanTa = 0;
            for (int i = 1; i < 6 && currColumn + i < MyConst._NumColumn; i++)
            {
                if (_listChessBoard[currRow][currColumn + i].Ownership == 1)
                {
                    soQuanTa++;
                    break;
                }
                else if (_listChessBoard[currRow][currColumn + i].Ownership == 2)
                {
                    soQuanDich++;
                }
                else
                    break;
            }

            for (int i = 1; i < 6 && currColumn - i >= 0; i++)
            {
                if (_listChessBoard[currRow][currColumn - i].Ownership == 1)
                {
                    soQuanTa++;
                    break;
                }
                else if (_listChessBoard[currRow][currColumn - i].Ownership == 2)
                {
                    soQuanDich++;
                }
                else
                    break;
            }
            if (soQuanTa == 2)
                return 0;
            diemTong += arrayDef[soQuanDich];
            //diemTong -= arrayAtt[soQuanTa];
            return diemTong;
        }
        long diemPTCheoChinh(int currRow, int currColumn)
        {
            long diemTong = 0;
            int soQuanDich = 0;
            int soQuanTa = 0;
            for (int i = 1; i < 6 && currRow + i < MyConst._NumRow && currColumn + i < MyConst._NumColumn; i++)
            {
                if (_listChessBoard[currRow + i][currColumn + i].Ownership == 1)
                {
                    soQuanTa++;
                    break;
                }
                else if (_listChessBoard[currRow + i][currColumn + i].Ownership == 2)
                {
                    soQuanDich++;
                }
                else
                    break;
            }

            for (int i = 1; i < 6 && currRow - i >= 0 && currColumn - i >= 0; i++)
            {
                if (_listChessBoard[currRow - i][currColumn - i].Ownership == 1)
                {
                    soQuanTa++;
                    break;
                }
                else if (_listChessBoard[currRow - i][currColumn - i].Ownership == 2)
                {
                    soQuanDich++;
                }
                else
                    break;
            }
            if (soQuanTa == 2)
                return 0;
            diemTong += arrayDef[soQuanDich];
           // diemTong -= arrayAtt[soQuanTa];
            return diemTong;
        }
        long diemPTCheoPhu(int currRow, int currColumn)
        {
            long diemTong = 0;
            int soQuanDich = 0;
            int soQuanTa = 0;
            for (int i = 1; i < 6 && currColumn + i < MyConst._NumColumn && currRow - i >= 0; i++)
            {
                if (_listChessBoard[currRow - i][currColumn + i].Ownership == 1)
                {
                    soQuanTa++;
                    break;
                }
                else if (_listChessBoard[currRow - i][currColumn + i].Ownership == 2)
                {
                    soQuanDich++;

                }
                else
                    break;
            }

            for (int i = 1; i < 6 && currRow + i < MyConst._NumRow && currColumn - i >= 0; i++)
            {
                if (_listChessBoard[currRow + i][currColumn - i].Ownership == 1)
                {
                    soQuanTa++;
                    break;
                }
                else if (_listChessBoard[currRow + i][currColumn - i].Ownership == 2)
                {
                    soQuanDich++;

                }
                else
                    break;
            }
            if (soQuanTa == 2)
                return 0;
            diemTong += arrayDef[soQuanDich];
           // diemTong -= arrayAtt[soQuanTa];
            return diemTong;
        }

        #endregion


        #endregion

        #region Mod game
        public int Mod2Player(int mouseX, int mouseY, Panel pnl, Label lb, PictureBox ptb)
        {
            if (chess(mouseX, mouseY))
            {
                if (checkWin())
                {
                    pnl.Enabled = false;
                    win();
                    return 1;
                }
            }
            else
                return -1;
            loadInfo(lb, ptb);
            return 0;
        }
        public int Mod1Player(int mouseX, int mouseY, Panel pnl, Label lb, PictureBox ptb)
        {
            if (chess(mouseX, mouseY))
            {
                if (checkWin())
                {
                    pnl.Enabled = false;
                    win();
                    return 1;
                }
                loadInfo(lb, ptb);
            }
            else
            {
                return -1;
            }
            startComputer();
            if (checkWin())
            {
                pnl.Enabled = false;
                win();
                return 1;
            }
            loadInfo(lb, ptb);
            return 0;
        }
        public void Undo(Label lbName, PictureBox ptb, Panel pnl)
        {
            if (StackChess.Count == 0 && mod == 2)
                return;
            if (StackChess.Count == 1 && mod == 1)
                return;
            if (mod == 1)
                undoMod1(pnl);
            else if (mod == 2)
                undoMod2(pnl);
            loadInfo(lbName, ptb);

        }

        void undoMod1(Panel pnl)
        {
            for (int i = 0; i < 2; i++)
                undoMod2(pnl);

        }
        void undoMod2(Panel pnl)
        {
            if (pnl == null)
                return;
            Chess_BoardInfo tmp = StackChess.Pop();
            tmp.Ownership = 0;
            tmp.FileImage = null;
            turn = turn != 1 ? 1 : 2;
            _grs.Clear(pnl.BackColor);
            chessboard.drawChessboard();
            drawChessBoardRefesh();
        }

        #endregion
    }

}
